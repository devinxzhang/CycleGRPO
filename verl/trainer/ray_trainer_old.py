...
class RayPPOTrainer:
    ...
    def _make_batch_data(self, metrics: dict[str, Any], task: str) -> DataProto:
        batch = None
        ...
        while True:
            ...
            # batch_dict: ['cap_input_ids', 'seg_input_ids', 'cap_attention_mask', 'seg_attention_mask', 'cap_position_ids', \
            # 'seg_position_ids', 'masks', 'source', 'cap_multi_modal_data', 'seg_multi_modal_data', 'cap_raw_prompt_ids', 'seg_raw_prompt_ids', 'cap_ground_truth', 'seg_ground_truth']
            new_batch: DataProto = DataProto.from_single_dict(batch_dict, meta_info=meta_info)
            # new_batch.batch: ['cap_input_ids', 'seg_input_ids', 'cap_attention_mask', 'seg_attention_mask', 'cap_position_ids', 'seg_position_ids']
            # new_batch.non_tensor_batch: ['masks', 'source', 'cap_multi_modal_data', 'seg_multi_modal_data', 'cap_raw_prompt_ids', 'seg_raw_prompt_ids', 'cap_ground_truth', 'seg_ground_truth']
            new_batch.non_tensor_batch["uid"] = np.array(
                [str(uuid.uuid4()) for _ in range(len(new_batch.batch))], dtype=object
            )

            # prepare gen_batch for generation of captioning / segmentation training
            task_dict = {k.replace(f'{task_id}_', ''): new_batch.batch.pop(k) for k in [f'{task_id}_input_ids', f'{task_id}_attention_mask', f'{task_id}_position_ids']}
            task_dict.update({k.replace(f'{task_id}_', ''): new_batch.non_tensor_batch.pop(k) for k in [f'{task_id}_raw_prompt_ids', f'{task_id}_multi_modal_data']})
            gen_batch = DataProto.from_single_dict(task_dict, meta_info=meta_info)
            gen_batch.meta_info.update({'task': task})

            # generate a batch
            gen_batch_output = self.actor_rollout_ref_wg.generate_sequences(gen_batch)
            # gen_batch_output.batch.keys(): ['prompts', 'responses', 'input_ids', 'attention_mask', 'response_mask', 'position_ids']
            # gen_batch_output.non_tensor_batch.keys(): ['multi_modal_data']
            # gen_batch_output.batch.update({f"{task_id}_{k}": gen_batch_output.batch.pop(k) for k in list(gen_batch_output.batch.keys())})
            # gen_batch_output.non_tensor_batch.update({f"{task_id}_{k}": gen_batch_output.non_tensor_batch.pop(k) for k in list(gen_batch_output.non_tensor_batch.keys())})

            if self.config.algorithm.adv_estimator == "remax":
                gen_baseline_batch = deepcopy(gen_batch)
                gen_baseline_batch.meta_info["temperature"] = 0
                gen_baseline_batch.meta_info["n"] = 6
            ...
            # new_batch.batch.keys(): []
            # new_batch.non_tensor_batch.keys(): ['masks', 'source', 'cap_ground_truth', 'seg_ground_truth', 'uid']
            new_batch = new_batch.union(gen_batch_output)
            # new_batch.batch.keys(): ['prompts', 'responses', 'input_ids', 'attention_mask', 'response_mask', 'position_ids']
            # new_batch.non_tensor_batch.keys(): ['masks', 'source', 'cap_ground_truth', 'seg_ground_truth', 'uid', 'multi_modal_data']
    ...
    def _make_seg_batch_data_for_caption(self, batch: DataProto) -> DataProto:
        ...
        for i in range(len(batch.non_tensor_batch['multi_modal_data'])):
            ...
            example = {'seg_problem': seg_problem, 
                        'images': batch.non_tensor_batch['seg_multi_modal_data'][i]['images'], 
                        ...
                        # 'seg_token_ground_truth': self.tokenizer.decode(batch.batch['prompts'][i], skip_special_tokens=True),
                        'cap_images': batch.non_tensor_batch['multi_modal_data'][i]['images']}
        ...
        gen_batch.non_tensor_batch.update({'images': gen_batch.non_tensor_batch['multi_modal_data']})
        ...
        gen_batch_output = self.actor_rollout_ref_wg.generate_sequences(gen_batch)
        # gen_batch_output.batch.keys(): ['prompts', 'responses', 'input_ids', 'attention_mask', 'response_mask', 'position_ids']
        # gen_batch_output.non_tensor_batch.keys(): ['multi_modal_data', 'mask_token_accuracy', 'format_correct', 'seg_responses', 'cap_responses', 'seg_gt']
        # fname = f"gen_batch_output_non_tensor_batch_{getattr(self, 'global_step', 0)}_{uuid.uuid4().hex}.pt"
        # torch.save(gen_batch_output.non_tensor_batch, os.path.join('rollout_analysis', fname))
        ...
        while self.global_step < self.training_steps:
            ...
            with timer("step", timing_raw):
                ...
                with timer("gen", timing_raw):
                    self.actor_rollout_ref_wg.prepare_rollout_engine()
                    batch = self._make_batch_data(metrics=metrics, task='caption')
                    self.actor_rollout_ref_wg.release_rollout_engine()
                ...
                if self.config.worker.actor.optimize_captioner:
                    # compute reward
                    with timer("gen", timing_raw):
                        batch.meta_info["n"] = 6
                        self.actor_rollout_ref_wg.prepare_rollout_engine()
                        batch, seg_batch = self._make_seg_batch_data_for_caption(batch)
                        batch.meta_info.pop("n", None)
                        self.actor_rollout_ref_wg.release_rollout_engine()
                    ...
                    with timer("adv", timing_raw):
                        if "token_level_scores" not in batch.batch:
                            ...
                            batch.batch["token_level_scores"] = reward_tensor  # [8, 8192]
                ...
                if self.config.worker.actor.optimize_segmenter:
                    ## Start Segmentation Session
                    # compute reward
                    if "token_level_scores" not in seg_batch.batch:
                        with timer("reward", timing_raw):
                            seg_reward_ref = self.reward_fn.compute_reward.remote(self.global_step, seg_batch, task='segmentation')
                    
                    # recompute old_log_probs
                    with timer("old", timing_raw):
                        old_log_probs = self.actor_rollout_ref_wg.compute_log_probs(seg_batch) 
                        seg_batch = seg_batch.union(old_log_probs)

                    # compute ref_log_probs
                    if self.use_reference_policy:
                        with timer("ref", timing_raw):
                            ref_log_probs = self.actor_rollout_ref_wg.compute_ref_log_probs(seg_batch)
                            seg_batch = seg_batch.union(ref_log_probs)

                    # compute values
                    if self.use_critic:
                        with timer("values", timing_raw):
                            values = self.critic_wg.compute_values(seg_batch)
                            seg_batch = seg_batch.union(values)

                    with timer("adv", timing_raw):
                        if "token_level_scores" not in seg_batch.batch:
                            # get token level scores asynchronously
                            reward_tensor, reward_metrics = ray.get(seg_reward_ref)
                            seg_batch.batch["token_level_scores"] = reward_tensor
                            reward_metrics = {f"reward/{k}": v for k, v in reduce_metrics(reward_metrics).items()}
                            metrics.update(reward_metrics)
                        ...
                        if not self.config.algorithm.use_kl_loss and self.use_reference_policy:
                            # apply kl penalty to reward
                            batch, kl_metrics = apply_kl_penalty(seg_batch, self.kl_ctrl, self.config.algorithm.kl_penalty)
                            metrics.update(kl_metrics)
                        else:
                            seg_batch.batch["token_level_rewards"] = seg_batch.batch["token_level_scores"]

                        # compute advantages, executed on the driver process
                        seg_batch = compute_advantage(
                            seg_batch,
                            adv_estimator=self.config.algorithm.adv_estimator,
                            gamma=self.config.algorithm.gamma,
                            lam=self.config.algorithm.lam,
                    ...
                    if self.use_critic:
                        with timer("update_critic", timing_raw):
                            critic_output = self.critic_wg.update_critic(seg_batch)

                        critic_metrics = reduce_metrics(critic_output.non_tensor_batch)
                        metrics.update(critic_metrics)
                    
                if self.config.worker.actor.optimize_segmenter and self.config.worker.actor.optimize_captioner:
                    # Case 1: Both tasks - Use gradient accumulation for batch and seg_batch
                    cap_batch_size = len(batch)
                    seg_batch_size = len(seg_batch)
                    ...
                    if self.config.trainer.critic_warmup <= self.global_step:
                        ...
                        with timer("update_actor", timing_raw):
                            ...
                            # Step 1: Accumulate gradients from batch (combined non_single + single)
                            if batch is not None and cap_batch_size > 0:
                                batch.meta_info['grad_weight'] = cap_grad_weight
                                batch.meta_info['global_batch_size_per_device'] = len(batch) // self.actor_rollout_ref_wg.world_size
                                cap_output = self.actor_rollout_ref_wg.accumulate_actor_gradients(batch)
                    ...
                    # Case 2: Only captioner - Use batch to update
                    if self.config.trainer.critic_warmup <= self.global_step:
                        self.actor_rollout_ref_wg.clear_multi_modal_cache()
