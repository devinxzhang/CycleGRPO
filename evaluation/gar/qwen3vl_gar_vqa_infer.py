...
def main():
    ...
    with open('<PATH_TO_GAR_BENCH>/GAR-Bench-VQA.json', 'r') as f:
    ...
    for eval_sample in eval_samples:
        ...
        # Ensure quant_codes is always a list of lists
        if quant_codes and isinstance(quant_codes[0], int):
            quant_codes = [quant_codes]

        region_quant_codes = []
        for region_idx in range(len(boxes)):
            region_quant_codes.append([depth_idx*CODEBOOK_SIZE+quant_code for depth_idx, quant_code in enumerate(quant_codes[region_idx])])

        region_quant_codes_str = []
        for region_idx in range(len(boxes)):
            region_quant_codes_str.append(MT_START_TOKEN + ''.join([MT_CONTEXT_TOKEN.format(str(code).zfill(4)) for code in region_quant_codes[region_idx]]) + MT_END_TOKEN)
        ...
        # Replace <Prompti> in question_str with region_quant_codes_str[i]
        import re
        def replace_prompti(match):
            idx = int(match.group(1))
            return region_quant_codes_str[idx] if idx < len(region_quant_codes_str) else match.group(0)
