...
def parse_args():
    parser = argparse.ArgumentParser(description='DenseWorld Rollout Evaluation')
    parser.add_argument(
        '--model_path',
...
IMAGE_FOLDER = 'rl_dataset/denseworld_5k_img_21219_single_target_samples_train.parquet'
...
def evaluate_sample(args, model, processor, vq_sam2, sam2_image_processor, table, sample_index, log_lines, CODEBOOK_SIZE, CODEBOOK_DEPTH):
    ...
    # Get image(s) — DenseWorld may store either in-memory bytes or file paths
    images = []
    for img_info in row['images']:
        # Case 1: dict with raw bytes
        if isinstance(img_info, dict) and 'bytes' in img_info:
            image_bytes = img_info['bytes']
            img = Image.open(io.BytesIO(image_bytes)).convert('RGB')
        else:
            # Case 2: direct path string or dict with path/filename key
            if isinstance(img_info, str):
                img_path = img_info
            elif isinstance(img_info, dict):
                img_path = img_info.get('path') or img_info.get('filename') or img_info.get('file')
            else:
                img_path = None

            if img_path is None:
                raise ValueError(f"Cannot determine image bytes or path from img_info: {img_info!r}")
    ...
    gt_mask = row['seg_answer']  # expected token string like '<answer><|mt_start|>...<|mt_end|></answer>'

    # Decode GT mask tokens into a binary mask using VQ-SAM2 (fall back to empty mask on failure)
    try:
        quant_ids = extract_mt_token_ids(gt_mask)
        if len(quant_ids) == 0:
            log(f"Warning: no mask tokens found in GT for sample {sample_index}")
            gt_mask_decoded = np.zeros((h, w), dtype=np.uint8)
        else:
            if len(quant_ids) % CODEBOOK_DEPTH != 0:
                gt_mask = fix_mt_format_comprehensive(gt_mask)
                quant_ids = extract_mt_token_ids(gt_mask)

            if len(quant_ids) % CODEBOOK_DEPTH != 0:
                log(f"Warning: GT mask token format error for sample {sample_index}")
                gt_mask_decoded = np.zeros((h, w), dtype=np.uint8)
            else:
                ...
                _pred_masks = (_pred_masks > 0.5).cpu().numpy().astype(np.uint8)
                merged_mask = np.any(_pred_masks[:, 0, :, :], axis=0).astype(np.uint8)
                gt_mask_decoded = merged_mask
    except Exception as e:
        log(f"Error decoding GT mask for sample {sample_index}: {e}")
        gt_mask_decoded = np.zeros((h, w), dtype=np.uint8)
    ...
    if os.path.exists(result_path):
        log(f"Sample {sample_index} already evaluated, skipping. (Found {result_path})")
        return
    ...
    all_results = {
        ...
        "source": row.get('source', 'denseworld'),
        "captions": []
    }

    log(f"\n{'='*60}")
    log(f"Evaluating sample {sample_index} (DenseWorld)")
    ...
    for cap_idx in range(args.cap_rollouts):
        ...
        # seg_problem = f"Please segment {cap_pred_clean}"
        ...
        for seg_idx in range(args.seg_rollouts):
            ...
            # Save pred mask overlaid on original image (blue) and binary mask
