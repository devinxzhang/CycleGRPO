...
def parse_args():
    ...
    parser.add_argument(
        '--dataset',
        default='../datasets/PaDT-MLLM/RefCOCO/refcocog_test.json',
...
def metric(args):
    ...
    print('Total cases: %d' % len(os.listdir(args.output)))
    for json_file in os.listdir(args.output):
        with open(os.path.join(args.output, json_file), 'r') as f:
    ...
    vlm_eval_ap = []
    vlm_eval_ciou = []
    # vlm_json_files = [args.dataset]
    vlm_json_files = [f"../datasets/PaDT-MLLM/RefCOCO/rec_jsons_processed/{args.dataset.split('/')[-1]}"]
...
def main():
    ...
    for data_dict in tqdm.tqdm(all_data_dict):
        ...
        if os.path.exists(f"{args.output}/{case_id}.json"):
            print("file exists.............")
            continue
        ...
        if len(quant_ids) == 0:
            ...
            with open(f"{args.output}/{case_id}.json", 'w') as f:
