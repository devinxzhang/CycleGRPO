...
def parse_args():
    ...
    parser.add_argument(
        '--dataset',
        default='../datasets/PaDT-MLLM/RefCOCO/grefcoco_val.json',
...
def metric():
    ...
    for json_file in os.listdir('./results/refcocog'):
        with open(os.path.join('./results/refcocog', json_file), 'r') as f:
...
def main():
    ...
    for data_dict in tqdm.tqdm(all_data_dict[_start_:_end_]):
        ...
        if os.path.exists(f"./results/refcocog/{case_id}.json"):
            print("file exists.............")
            continue
        ...
        if len(quant_ids) == 0:
            ...
            with open(f"./results/refcocog/{case_id}.json", 'w') as f:
