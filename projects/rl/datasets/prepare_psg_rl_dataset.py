...
def main():
    random.seed(42)
    psg_ann_file = "./data/tokenmask_data_256x2_cot_format/mask_generation_psg45k_v1.json"
    with open(psg_ann_file, 'r') as f:
    ...
    for data_dict in json_data[:200]:
        ret_data_dict = copy.deepcopy(data_dict)
        ret_data_dict.update({'source': 'psg', 'segmentations': []})
    ...
    dataset["train"].to_parquet(f"rl_dataset/psgv1_{len(shard_items)//1000}k_train.parquet")
