...
def main():
    random.seed(42)
    ric_ann_file = "./data/tokenmask_data_256x2_cot_format/mask_generation_padt_refcoco_direction_order_100k.json"
    ...
    shard_items = []
    for data_dict in json_data[:1000]:
        ret_data_dict = copy.deepcopy(data_dict)
        ret_data_dict.update({'source': 'thinking_res', 'segmentations': []})
    ...
    dataset["train"].to_parquet(f"rl_dataset/res_thinking_direction_order_{len(shard_items)//1000}k_train.parquet")
