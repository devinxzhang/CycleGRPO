...
def main():
    random.seed(42)
    gcg_ann_file = "./data/tokenmask_data_256x2/mask_generation_gcg_exclude_grandf195k.json"
    with open(gcg_ann_file, 'r') as f:
        json_data = json.load(f)

    random.shuffle(json_data)
    
    shard_items = []
    for data_dict in json_data[:40000]:
        ...
        assert from_human[-1] == '.'
        new_from_human = from_human[:-1] + ' and highlight the phrases.'
        conversations = []
        conversations.append({'from': 'human', 'value': new_from_human})
    ...
    dataset["train"].to_parquet(f"zhouyik_1028/syngcg{len(shard_items)//1000}k_train.parquet")
