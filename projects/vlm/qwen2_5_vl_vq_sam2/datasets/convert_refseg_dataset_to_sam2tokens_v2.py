...
def main():
    ...
    temp_save_root = "./temp_data_256x2_0927/ref_seg_v2/refclef"
    ...
    #======================load dataset
    refer_api = REFER('./data/ref_seg/refclef/saiapr_tc-12', './data/ref_seg/refclef/refs(unc).p', './data/ref_seg/refclef/instances.json', dataset="refclef")
    ...
    for item in loaded_images:
        item = item.copy()
        item["file_name"] = os.path.join('./data/ref_seg/refclef/saiapr_tc-12', item["file_name"])
    ...
    for index in tqdm.tqdm(range(len(refer_seg_ds['images']))):
        ...
        for sent, binary_masks in zip(sents, anno_masks):
            assert len(binary_masks) == 1
            if np.sum(binary_masks[0]) < 1.0:
                print("===========>Encounter empty mask.")
                continue
