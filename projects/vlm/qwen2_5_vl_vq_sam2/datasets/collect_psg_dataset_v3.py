...
def main(task_id):
    ...
    temp_save_root = "./temp_data_256x2_0927/psg_givenmask"
    if not os.path.exists(temp_save_root):
        os.makedirs(temp_save_root)
    dataset_name = "psg_givenmask"
    ...
    for data_info in tqdm.tqdm(data_infos[_start_:_end_]):
        ...
        candidate_mask_str = ', '.join(list(segment_id_2_mask_tokens.values()))
        

        # second round QA
        question = "<image>\n" + candidate_str + 'Given masks: ' + candidate_mask_str + '\n' + random.choice(QUESTIONS)
        answer = "```json\n[{mask_2d}]\n```"
        mask_2d_str = ''
