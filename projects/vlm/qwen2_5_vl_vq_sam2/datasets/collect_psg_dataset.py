...
def main(task_id):
    ...
    temp_save_root_v1 = "./temp_data_256x2_0927/psg_v1"
    if not os.path.exists(temp_save_root_v1):
        os.makedirs(temp_save_root_v1)
    dataset_name = "psg_v1"

    temp_save_root_v2 = "./temp_data_256x2_0927/psg_v2"
    if not os.path.exists(temp_save_root_v2):
        os.makedirs(temp_save_root_v2)
    dataset_name = "psg_v2"
    ...
    for data_info in tqdm.tqdm(data_infos[_start_:_end_]):
        ...
        question_v2 = "<image>\n" + candidate_str + random.choice(QUESTIONS)
        ...
        for triplet in relation_annos:
            ...
            masks = torch.stack([torch.from_numpy(np.ascontiguousarray(x.copy())) for x in [sub_mask, obj_mask]])
            try:
            ...
            sub_item_str = "{\"mask_2d\": " + "\"" + sam2token_list[0] + "\"" + ", \"label\": \"" + sub_category + "\"}"
            obj_item_str = "{\"mask_2d\": " + "\"" + sam2token_list[1] + "\"" + ", \"label\": \"" + obj_category + "\"}"
        ...
        conversation_v1= []
        conversation_v1.append({'from': 'human', 'value': question_v1})
        conversation_v1.append({'from': 'gpt', 'value': answer})

        ret_data_dict_v1 = {
            'image': image_path,
            'conversations': conversation_v1,
        }
        shard_items_v1.append(ret_data_dict_v1)
        count += 1

        if count % shard_size == 0:
            shard_idx += 1
            out_path = os.path.join(temp_save_root_v1, f"{dataset_name}-segment-{shard_idx:05d}-chunk{task_id}.json")
            with open(out_path, "w") as f:
                json.dump(shard_items_v1, f)
            shard_items_v1.clear()
            print(f"[SAVE] {out_path} ({count} items)", flush=True)

        conversation_v2= []
        conversation_v2.append({'from': 'human', 'value': question_v2})
        conversation_v2.append({'from': 'gpt', 'value': answer})

        ret_data_dict_v2 = {
            'image': image_path,
            'conversations': conversation_v2,
        }
        shard_items_v2.append(ret_data_dict_v2)

        if count % shard_size == 0:
            out_path = os.path.join(temp_save_root_v2, f"{dataset_name}-segment-{shard_idx:05d}-chunk{task_id}.json")
            with open(out_path, "w") as f:
                json.dump(shard_items_v2, f)
            shard_items_v2.clear()
            print(f"[SAVE] {out_path} ({count} items)", flush=True)
