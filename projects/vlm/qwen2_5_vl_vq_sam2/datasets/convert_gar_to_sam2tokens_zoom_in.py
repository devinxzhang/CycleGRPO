...
def main(task_id):
    ...
    temp_save_root = "./temp_data_256x2_0927/gar_zoom_in/"
    if not os.path.exists(temp_save_root):
        os.makedirs(temp_save_root)
    dataset_name = "gar_zoom_in"
    ...
    for part_id in range(1, 11):
        sub_dataset = load_from_disk(f'./data/HaochenWang/Grasp-Any-Region-Dataset/Fine-Grained-Dataset-Part{part_id}')
        all_shards.append(sub_dataset)
    ...
    for row_id in tqdm.tqdm(list(range(rows))[_start_:_end_]):
        ...
        if data_dict['conversations'][1]['from'] != 'gpt':
        ...
        ret_data_dict = {
            'image': [image_path, cropped_image_dict],
            'conversations': conversation,
        }
