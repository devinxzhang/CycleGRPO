...
def main(task_id):
    ...
    for index in tqdm.tqdm(list(range(len(json_data)))[_start_:_end_]):
        ...
        if os.path.exists(os.path.join(temp_save_root, f"{image_id}_{index}_grandf.json")):
            print("file exists..................")
            continue
        ...
        # output_image.save(f"test_grandf_{image_id}.jpg")
        # if index < 100:
        #     continue
        # else:
        #     exit(0)
        ...
        # with open(os.path.join(temp_save_root, f"{image_id}_{index}_grandf.json"), 'w') as f:
