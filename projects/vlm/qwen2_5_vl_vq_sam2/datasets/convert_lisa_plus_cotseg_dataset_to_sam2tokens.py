...
def main(task_id):
    ...
    temp_save_root = "./temp_data_256x2_0927/lisa_plus/cot_seg"
    ...
    with open("./data/Senqiao/LISA_Plus_COT/data.json", 'r') as f:
        data_list = json.load(f)

    dataset_name = "lisa_plus_cot"
    ...
    for index in tqdm.tqdm(list(range(len(data_list)))[start:end]):
        ...
        answer = data_dict['English Answer']

        image_path = os.path.join("./data/coco/train2017", data_dict['image_path'])
        ...
        anno_ids, target_annos = [], []
        for anno_id in data_dict["info"].keys():
            anno_ids.append(anno_id)
            target_annos.append(coco_reader.loadAnns([int(anno_id)]))
        ...
        pattern = r'\[\u300a(\d+)\|([^\u300b]+)\u300b\]'
        def replace_instance_markers(match):
            instance_id = match.group(1)  
            label = match.group(2) 

            try:
                instance_index = anno_ids.index(instance_id)
                sam2_tokens = sam2_tokens_list[instance_index]
                return sam2_tokens
            except ValueError:
                return match.group(0)
        
        answer = re.sub(pattern, replace_instance_markers, answer)
