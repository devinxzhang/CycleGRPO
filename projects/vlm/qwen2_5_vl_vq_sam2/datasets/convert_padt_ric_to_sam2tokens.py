...
def main(task_id):
    ...
    dataset_name = "padt_ric"
    temp_save_root = "./temp_data_256x2_0927/padt_ric/"
    ...
    all_data_dict = []
    with open('<PATH_TO_DATA>/discrete_spatial_tokenizer/data/PaDT-MLLM/ReferringImageCaptioning/ric_instances_train2017.json', 'r') as f:
        for line in f:
            # Skip empty lines
            if line.strip():
    ...
    for data_dict in tqdm.tqdm(all_data_dict[_start_:_end_]):
        ...
        for obj_id, instance in enumerate(instances):
            ...
            obj_mt_dict[f'<|Obj_{obj_id}|>'] = sam2_tokens
        
        question = "<image>\n" + data_dict['conversations'][0]['value']
        raw_answer = data_dict['answer_template']
        for obj_id, mask_token in obj_mt_dict.items():
            if f'({obj_id})' in raw_answer:
                raw_answer = raw_answer.replace(f'({obj_id})', mask_token)
            elif obj_id in raw_answer:
                raw_answer = raw_answer.replace(obj_id, mask_token)
            else:
                raise NotImplementedError
        
        # all_binary_masks = np.stack(all_binary_masks, axis=0)
        # output_image = visualize(image, all_binary_masks, list(obj_mt_dict.keys()))
        # output_image.save('./padt_ric.jpg')
        
        conversation = []
        conversation.append({'from': 'human', 'value': question})
        conversation.append({'from': 'gpt', 'value': raw_answer})
