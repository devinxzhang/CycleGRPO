...
def main(task_id):
    ...
    pano_image_root = "<PATH_TO_DATA>/segmentation_datasets/coconut/xdeng77/coconut_xlarge/coconut_xlarge/panseg/"
    anno_file_root = "<PATH_TO_DATA>/segmentation_datasets/coconut/xdeng77/coconut_xlarge/coconut_xlarge/panseg_info/"
    coconut_large_anno_file = "<PATH_TO_DATA>/segmentation_datasets/coconut/xdeng77/coconut_large/panseg_object365_train_v2.json"
    with open(coconut_large_anno_file, 'r') as f:
        coconut_large_anno_data = json.load(f)
    
    categories = coconut_large_anno_data['categories']
    ...
    with open('coconut_xlarge_anno_info.json', 'r') as f:
        anno_file_list = json.load(f)

    rows = len(anno_file_list)
    chunk_size = (rows+31) // 32
    _start_ = task_id * chunk_size
    _end_ = _start_ + chunk_size
    _end_ = rows if _end_ > rows else _end_

    for index in tqdm.tqdm(list(range(rows))[_start_:_end_]):
        anno_file = anno_file_list[index]
