...
def main(task_id):
    ...
    # data_path = "<PATH_TO_DATA>/invig_data/invig21k_train_anns.jsonl"
    # data_item_list = []

    # # with open('<PATH_TO_DATA>/discrete_spatial_tokenizer/data/invig_info.json', 'r') as f:
    # #     filename_2_filepath = json.load(f)
    ...
    #             questions = item['ann']['questions']
    #             answers = item['ann']['answers']
    #             ref_bboxes = item['ann']['ref_bboxes']

    #             assert len(questions) == len(answers)

    #             history_dialog = "Role A: " + item['ann']['ref_exp'] + '\n\n'
    #             for q, a in zip(questions, answers):
    #                 history_dialog += "Role B: " + q + '\n\n'
    #                 history_dialog += "Role A: " + a + '\n\n'

    #             # if filename not in filename_2_filepath:
    #             #     print("skip ", filename)
    #             #     continue
    #             # data_item = {
    #             #     'image_file': filename_2_filepath[filename],
    #             #     'history_dialog': history_dialog,
    #             #     'ref_bboxes': ref_bboxes,
    #             # }
    #             data_item = {
    #                 'image_file': os.path.join('./data/invig_data/imgs', filename),
    #                 'history_dialog': history_dialog,
    #                 'ref_bboxes': ref_bboxes,
    #             }
    #             data_item_list.append(data_item)

    # with open('./data/invig21k_chat_info.json', 'w') as f:
    #     json.dump(data_item_list, f)
    ...
    temp_save_root = "./temp_data_256x2_0927/ref_seg/invig500k"
    ...
    data_path = "./data/invig_chat_info.json"
    dataset_name = 'invig500k'
    ...
    for item in tqdm(anno_data[start:end]):
        if count < skip_count:
            count += 1
            continue
        image_file = item['image_file']
        history_dialog = item['history_dialog']
        ref_bboxes = item['ref_bboxes']
        ...
        bbox_x1y1x2y2_np = np.array(ref_bboxes)
        num_ins = len(bbox_x1y1x2y2_np)
        sam2_pixel_values = sam2_pixel_values.repeat(num_ins, 1, 1, 1)
        ...
        # output_image = visualize(image, sam_masks, ['']*len(sam_masks))
        # output_image.save("invig21k_box2mask.jpg")
        ...
        question = "<image>\nPlease locate the image area discussed in the following conversation and respond with corresponding segmentation maps. \nDialogue: \n" + history_dialog
        ...
        for _quant_codes_ in quant_codes:
            ...
            item_str = "{\"mask_2d\": " + sam2_tokens + ", \"label\": \"image area discussed\"}"
