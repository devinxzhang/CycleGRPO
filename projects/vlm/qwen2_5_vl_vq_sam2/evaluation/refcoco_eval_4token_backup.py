    ...
    for idx in tqdm.tqdm(per_rank_ids):
        ...
        for text_idx, text in enumerate(texts):
            ...
            if len(quant_ids) == 0:
                pred_masks.append(None)
                continue
            elif len(quant_ids) > 4:
                quant_ids = quant_ids[:4]
            ...
            if iou[0][0].item() < 1.1:
                ...
                with torch.no_grad():
                    ...
                    #======two token
                    one_gt_quant_codes = [gt_quant_codes[0], -1, -1, -1]
                    ...
                    #=======pred first token
                    two_pred_quant_codes = [quant_ids[0], -1, -1, -1]
                    ...
                    #=======pred second token
                    second_pred_quant_codes = [-1, quant_ids[1], -1, -1]
                    ...
                    #=======pred third token
                    third_pred_quant_codes = [-1, -1, quant_ids[2], -1]
                    third_pred_quant_codes = torch.LongTensor(third_pred_quant_codes).to(vq_sam2.device).unsqueeze(0)
                    third_pred_quant_masks = vq_sam2.forward_with_codes(sam2_pixel_values, third_pred_quant_codes)
                    third_pred_quant_masks = torch.nn.functional.interpolate(third_pred_quant_masks, size=(ori_height, ori_width), mode='bilinear')
                    third_pred_quant_masks = third_pred_quant_masks > 0.5
                    third_pred_quant_masks = third_pred_quant_masks[:, 0, :, :].cpu().numpy().astype(np.uint8)

                    #=========pred fourth token
                    fourth_pred_quant_codes = [-1, -1, -1, quant_ids[3]]
                    fourth_pred_quant_codes = torch.LongTensor(fourth_pred_quant_codes).to(vq_sam2.device).unsqueeze(0)
                    fourth_pred_quant_masks = vq_sam2.forward_with_codes(sam2_pixel_values, fourth_pred_quant_codes)
                    fourth_pred_quant_masks = torch.nn.functional.interpolate(fourth_pred_quant_masks, size=(ori_height, ori_width), mode='bilinear')
                    fourth_pred_quant_masks = fourth_pred_quant_masks > 0.5
                    fourth_pred_quant_masks = fourth_pred_quant_masks[:, 0, :, :].cpu().numpy().astype(np.uint8)

                    #=======second token
                    second_gt_quant_codes = [-1, gt_quant_codes[1], -1, -1]
                    ...
                    #=======third token
                    third_gt_quant_codes = [-1, -1, gt_quant_codes[2], -1]
                    third_gt_quant_codes = torch.LongTensor(third_gt_quant_codes).to(vq_sam2.device).unsqueeze(0)
                    third_gt_quant_masks = vq_sam2.forward_with_codes(sam2_pixel_values, third_gt_quant_codes)
                    third_gt_quant_masks = torch.nn.functional.interpolate(third_gt_quant_masks, size=(ori_height, ori_width), mode='bilinear')
                    third_gt_quant_masks = third_gt_quant_masks > 0.5
                    third_gt_quant_masks = third_gt_quant_masks[:, 0, :, :].cpu().numpy().astype(np.uint8)

                    #=========fourth token
                    fourth_gt_quant_codes = [-1, -1, -1, gt_quant_codes[3]]
                    fourth_gt_quant_codes = torch.LongTensor(fourth_gt_quant_codes).to(vq_sam2.device).unsqueeze(0)
                    fourth_gt_quant_masks = vq_sam2.forward_with_codes(sam2_pixel_values, fourth_gt_quant_codes)
                    fourth_gt_quant_masks = torch.nn.functional.interpolate(fourth_gt_quant_masks, size=(ori_height, ori_width), mode='bilinear')
                    fourth_gt_quant_masks = fourth_gt_quant_masks > 0.5
                    fourth_gt_quant_masks = fourth_gt_quant_masks[:, 0, :, :].cpu().numpy().astype(np.uint8)


                output_image = visualize(image, _pred_masks, ['']*_pred_masks.shape[0])
                output_image.save(f'./coco_val_recon_7b/{img_id}_{uuid_str}_{iou_str}_pred_{pred_quant_ids_str}.jpg')

                output_image = visualize(image, target_masks[text_idx:text_idx+1], [''])
                output_image.save(f'./coco_val_recon_7b/{img_id}_{uuid_str}_{iou_str}_gt.jpg')

                output_image = visualize(image, reconstruct_masks, ['']*reconstruct_masks.shape[0])
                output_image.save(f'./coco_val_recon_7b/{img_id}_{uuid_str}_{iou_str}_reco_{gt_quant_codes_str}.jpg')

                output_image = visualize(image, one_token_pred_masks, ['']*one_token_pred_masks.shape[0])
                output_image.save(f'./coco_val_recon_7b/{img_id}_{uuid_str}_{iou_str}_gt_firsttoken.jpg')

                output_image = visualize(image, second_gt_quant_masks, ['']*second_gt_quant_masks.shape[0])
                output_image.save(f'./coco_val_recon_7b/{img_id}_{uuid_str}_{iou_str}_gt_secondtoken.jpg')

                output_image = visualize(image, third_gt_quant_masks, ['']*third_gt_quant_masks.shape[0])
                output_image.save(f'./coco_val_recon_7b/{img_id}_{uuid_str}_{iou_str}_gt_thirdtoken.jpg')

                output_image = visualize(image, fourth_gt_quant_masks, ['']*fourth_gt_quant_masks.shape[0])
                output_image.save(f'./coco_val_recon_7b/{img_id}_{uuid_str}_{iou_str}_gt_fourthtoken.jpg')

                output_image = visualize(image, two_pred_token_masks, ['']*two_pred_token_masks.shape[0])
                output_image.save(f'./coco_val_recon_7b/{img_id}_{uuid_str}_{iou_str}_pred_firsttoken.jpg')

                output_image = visualize(image, second_pred_quant_masks, ['']*second_pred_quant_masks.shape[0])
                output_image.save(f'./coco_val_recon_7b/{img_id}_{uuid_str}_{iou_str}_pred_secondtoken.jpg')

                output_image = visualize(image, third_pred_quant_masks, ['']*third_pred_quant_masks.shape[0])
                output_image.save(f'./coco_val_recon_7b/{img_id}_{uuid_str}_{iou_str}_pred_thirdtoken.jpg')

                output_image = visualize(image, fourth_pred_quant_masks, ['']*fourth_pred_quant_masks.shape[0])
                output_image.save(f'./coco_val_recon_7b/{img_id}_{uuid_str}_{iou_str}_pred_fourthtoken.jpg')

                with open(f'./coco_val_recon_7b/{img_id}_{uuid_str}_{iou_str}_gt.txt', 'w', encoding='utf-8') as file:
                    file.write(question)
