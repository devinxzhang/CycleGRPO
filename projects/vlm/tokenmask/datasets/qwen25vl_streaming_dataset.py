...
from projects.vlm.tokenmask.datasets.rope2d import get_rope_index_25, get_rope_index_2
...
import bisect
class MultiArrowDataset:
    def __init__(self, part_dirs, with_pil=False):
        self.part_dirs = part_dirs
        # 预加载“目录句柄”和长度，但不加载样本
        self._parts = [load_from_disk(p) for p in part_dirs]
        if with_pil:
            self._parts = [d.with_format("pil") for d in self._parts]
        self._cum_sizes = []
        c = 0
        for d in self._parts:
            c += len(d)
            self._cum_sizes.append(c)
    def __len__(self):
        return self._cum_sizes[-1] if self._cum_sizes else 0
    def _locate(self, idx):
        # 全局 idx -> 第几个分片 与 分片内索引
        part_idx = bisect.bisect_right(self._cum_sizes, idx)
        prev_cum = 0 if part_idx == 0 else self._cum_sizes[part_idx-1]
        local_idx = idx - prev_cum
        return part_idx, local_idx
    def __getitem__(self, idx):
        part_idx, local_idx = self._locate(idx)
        return self._parts[part_idx][local_idx]

class Qwen25VLStreamingDataset(Dataset):
    ...
    def __init__(
        ...
        super(Qwen25VLStreamingDataset, self).__init__()

        streaming_dataset_path = data_args.dataset_use
        shard_dirs = []
        for shard_name in os.listdir(streaming_dataset_path):
            if 'segment-chunk' in shard_name:
                shard_dirs.append(os.path.join(streaming_dataset_path, shard_name))

        self.list_data_dict = MultiArrowDataset(shard_dirs, with_pil=False)

        rank0_print(f"Loading datasets: {streaming_dataset_path}")
