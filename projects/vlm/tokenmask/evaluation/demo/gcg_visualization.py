...
def main():
    ...
    image_root = "<PATH_TO_DATA>/MaskTokenizer/data/coco/val2017"
    for image_file in tqdm.tqdm(list(os.listdir(image_root))):
        ...
        bg_img = Image.new("RGB", (ori_width, ori_height), color="white")
        # print("(ori_height, ori_width): ", (ori_height, ori_width))
        # print("bg_img.size: ", bg_img.size)
        # exit(0)
        output_image = visualize(bg_img, _pred_masks, tags)
        output_image.save(f'./CVPR2026/demo_gcg_white_bg/{image_name}.jpg')

        with open(f'./CVPR2026/demo_gcg_white_bg/{image_name}.txt', "w", encoding="utf-8") as ftxt:
