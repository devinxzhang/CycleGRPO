...
import cv2
def polygons_to_mask_opencv(json_data, target_label=None, fill_value=1, dtype=np.uint8):
    ...
    for shape in json_data['shapes']:
        ...
        if target_label is not None and label != target_label:
            continue
        pts = np.array(shape['points'], dtype=np.float32)  # shape: (N, 2), [x, y]
        # round+clip，避免越界和小数导致的锯齿问题
        pts = np.round(pts).astype(np.int32)
        pts[:, 0] = np.clip(pts[:, 0], 0, W - 1)
        pts[:, 1] = np.clip(pts[:, 1], 0, H - 1)
        # cv2.fillPoly 需要形状为 (num_polygons, num_points, 2)
...
def main():
    ...
    with open('<PATH_TO_DATA>/MaskTokenizer/CVPR2026/gui.json', 'r') as f:
        json_data = json.load(f)
        gt_masks = polygons_to_mask_opencv(json_data)
        image_path = "./CVPR2026/gui.png"
        ...
        for idx, gt_mask in enumerate(gt_masks):
            masks = torch.stack([torch.from_numpy(np.ascontiguousarray(gt_mask.copy()))])
            ...
            gt_mask_output = visualize(image, [gt_mask], [''])
            recon_mask_output = visualize(image, pred_masks, [''])
            gt_mask_output.save(f'./CVPR2026/gui_{idx}_gt.jpg')
            recon_mask_output.save(f'./CVPR2026/gui_{idx}_recon.jpg')
