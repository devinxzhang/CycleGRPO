...
def main():
    ...
    for data_dict in tqdm.tqdm(annotations):
        ...
        with torch.no_grad():
            ...
            quant_codes = vq_sam2_output.quant_codes.detach().squeeze().cpu().numpy().tolist()
        ...
        output_image_gt.save(f'./CVPR2026/entityseg_visualization/{count_str}_<{remap_quant_codes[0]}-{remap_quant_codes[1]}>_gt.jpg')
        output_image_pred.save(f'./CVPR2026/entityseg_visualization/{count_str}_<{remap_quant_codes[0]}-{remap_quant_codes[1]}>_pred.jpg')
        count += 1
        # exit(0)
