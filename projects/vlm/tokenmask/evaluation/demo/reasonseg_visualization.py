...
def main():
    ...
    for image_path, json_path in zip(images, jsons):
        ...
        try:
            output_image = visualize(image, multi_masks, ['']*len(multi_masks), color=None)
        except:
            continue
        count_str = f"{count}".zfill(4)
        output_image.save(f'./CVPR2026/demo_reasonseg_test/{count_str}_{image_name}.jpg')

        with open(f'./CVPR2026/demo_reasonseg_test/{count_str}_{image_name}.txt', "w", encoding="utf-8") as ftxt:
