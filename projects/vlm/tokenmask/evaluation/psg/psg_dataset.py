...
from projects.vlm.tokenmask.evaluation.psg.sgg_eval import sgg_evaluation
...
class PanopticSceneGraphDataset(CocoPanopticDataset):
    def __init__(
        ...
        # Load dataset
        dataset = load(ann_file)

        image_id_2_annos = {}
        for item in dataset['annotations']:
            image_id_2_annos[item['image_id']] = item
        
        self.image_id_2_annos = image_id_2_annos

        categories = dataset['categories']
        self.is_thing_dict = {item['id']: item['isthing'] for item in categories}
        ...
        # self.rel2freq = dict()
        # for i in range(len(dataset['relations_categories'])):
        #     self.rel2freq[i+1] = 0
        # self.so_rel2freq = dict()
        # for k in range(len(dataset['categories'])):
        #     for j in range(len(dataset['categories'])):
        #         for i in range(len(dataset['relations_categories'])):
        ...
        self.PREDICATES = [rel['name'] for rel in dataset['relations_categories']]
    ...
    def get_ann_info(self, image_id):
        image_annos = self.image_id_2_annos[image_id]
        ...
        if self.all_bboxes:
            ...
            gt_bboxes = np.array([a['bbox'] for a in image_annos['segments_info']],
                                 dtype=np.float32)
            gt_labels = np.array([a['category_id'] for a in image_annos['segments_info']],
                                 dtype=np.int64)
            ...
            for a in image_annos['segments_info']:
                # NOTE: Only thing bboxes are loaded
                if self.is_thing_dict[a['category_id']]:
        ...
        gt_mask_infos = []
        for s in image_annos['segments_info']:
            gt_mask_infos.append({
                'id': s['id'],
                'category': s['category_id'],
                'is_thing': self.is_thing_dict[s['category_id']]
            })

        # Process relationship annotations
        gt_rels = image_annos['relations'].copy()
        ...
        pan_seg_file_name = os.path.join('./data/coco/annotations', image_annos['file_name'])
        ...
        ann = dict(
            ...
            masks=gt_mask_infos,
            seg_map=pan_seg_file_name,
            segment_infos=image_annos['segments_info'],
        )
        # if self.split == 'train':
        ...
        **kwargs,
        ...
        if len(sg_metrics) > 0:
            ...
            for img_id in image_ids:
                ...
                gt_img = read_image(ann['seg_map'], format='RGB')
