...
class GroundingEvaluator:
    ...
    def evaluate(self, pred_folder: str, output_file: str = None):
        ...
        for json_file in os.listdir(pred_folder):
            if not json_file.endswith('.json'):
                continue
            with open(os.path.join(pred_folder, json_file), 'r') as f:
                item = json.load(f)
        ...
        model_name = "groundingsuite_model"
...
def main():
    ...
    parser.add_argument('--image_dir', type=str, default='./data/GroundingSuiteEval', help='Image directory')
    parser.add_argument('--gt_file', type=str, default='./data/GroundingSuiteEval/GroundingSuite-Eval.jsonl', help='Ground truth JSONL file path')
    parser.add_argument('--pred_folder', type=str, default='claude_predictions.jsonl', help='Model prediction JSONL file path')
    ...
    # Evaluate model
    if args.pred_folder:
        output_file = args.output_file or f"{os.path.basename(args.pred_folder).split('.')[0]}_result.json"
        evaluator.evaluate(
