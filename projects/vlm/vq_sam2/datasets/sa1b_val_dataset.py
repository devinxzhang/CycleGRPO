...
class SA1BValDataset(Dataset):
    ...
    def prepare_mask_input(self, image_file, masks):
    ...
    def prepare_data(self, index, fine=False):
        ...
        if not self.multi_targets:
            ...
            # if len(filtered_indices) == 0:
            #     return None
            # segms = [segms[random_idx] for random_idx in filtered_indices]

            random_indices = list(range(len(segms)))
            random.shuffle(random_indices)
            segms = [segms[random_idx] for random_idx in random_indices if random_idx not in filtered_indices]
        else:
            if len(segms) > 10:
