...
class QWEN25VL_TokenMask(BaseModel):
    def __init__(
        self,
        qwen25vl_hf_model,
        vqsam2_hf_model,
        ...
        super(QWEN25VL_TokenMask, self).__init__()
        ...
        self.seg_token_id = self.tokenizer.convert_tokens_to_ids('<|mt_start|>')
    ...
    def state_dict(self, *args, **kwargs):
        ...
        to_return.update(
            {k: v
             for k, v in state_dict.items() if 'vqsam2_model.' in k})
    ...
    def forward(self, data, data_samples=None, mode='loss'):
        ...
        seg_token_mask = labels == self.seg_token_id

        _zero = hidden_states.mean() * 0.0
        loss_recon, loss_quant = 0.0, 0.0
        if gt_masks is None:
            for n, p in self.vqsam2_model.model.sam2_model.sam_mask_decoder.named_parameters():
                _zero += p.sum() * 0.0
            loss_recon += _zero
            loss_quant += _zero
        else:
            ...
            pred_embeddings = pred_embeddings.reshape(-1, 1, C)
            MAX_MASKS_IN_BATCH = 40
            for depth_idx in range(1, 2):
                sam2_pixel_values_per_depth = sam2_pixel_values[:MAX_MASKS_IN_BATCH]
                pred_mt_embeds_per_depth = pred_embeddings[:MAX_MASKS_IN_BATCH, :depth_idx, :]
                # discrete contiguous seg embeddings
                vq_sam2_output = self.vqsam2_model(
                    sam2_pixel_values_per_depth, pred_mt_embeds_per_depth,
                    gt_masks=gt_masks, reconstruct_mask=True, freeze_codebook=self.freeze_codebook)
                loss_recon += vq_sam2_output['loss_recon']
                loss_quant += vq_sam2_output['loss_quant']
