...
class QWEN25VL_VQSAM2Model(BaseModel):
    ...
    def forward(self, data, data_samples=None, mode='loss'):
        ...
        # loss_dict = {
        #     'llm_loss': qwen25vl_output.loss + _zero,
        # }
        ...
        # repa_loss = 0.0
        # if not self.repa_loss or codebook_embeds is None:
        #     repa_loss += _zero
        # else:
        #     special_token_ids = list(range(self.mt_start_token_id + 1, self.mt_end_token_id))
        #     input_embeds = self.qwen25vl_model.get_input_embeddings()
        #     special_token_ids = torch.LongTensor(special_token_ids).to(input_embeds.weight.device)
        #     mt_embeds = input_embeds(special_token_ids)
        #     mt_embeds = self.llm_to_sam(mt_embeds.unsqueeze(0)).squeeze(0)
        #     codebook_embeds = torch.cat(codebook_embeds, dim=0).to(mt_embeds.dtype).to(mt_embeds.device)
            
        #     dot_product = (mt_embeds * codebook_embeds).sum(dim=-1)
        #     norm_A = torch.norm(mt_embeds, dim=-1)
        #     norm_B = torch.norm(codebook_embeds, dim=-1)
        #     cos_sim = dot_product / (norm_A * norm_B + 1e-8)
        #     repa_loss = (1-cos_sim).mean() * 0.2
